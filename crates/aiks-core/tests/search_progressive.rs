...
use aiks_core::{
    CreateKnowledgeInput, KnowledgeService, UnifiedSearchFilter, UnifiedSearchService,
};
...
async fn lexical_callback_arrives_without_waiting_for_embedding() {
    ...
    let task = tokio::spawn(async move {
        service
            .search_with_progress("LLM", 10, UnifiedSearchFilter::default(), move |partial| {
                if let Some(sender) = sender.take() {
                    let _ = sender.send(partial.clone());
                }
            })
            .await
    });
    let partial = tokio::time::timeout(Duration::from_secs(2), receiver)
        .await
        .unwrap()
        .unwrap();
...
async fn cjk_technical_identifiers_and_filters_survive_indexed_recall() {
    ...
    for query in [
        ...
        let mut seen = false;
        let search =
            service.search_with_progress(query, 10, UnifiedSearchFilter::default(), |partial| {
    ...
    let mut count = None;
    let search = service.search_with_progress(
        "LLM",
        10,
        UnifiedSearchFilter {
