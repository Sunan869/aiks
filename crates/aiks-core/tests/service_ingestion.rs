...
fn registration_and_upload_retry_are_idempotent() {
    ...
    assert_eq!(
        registration,
        store
            .register_source(&ctx, SourceKind::Continue, "device-1")
            .unwrap()
    );
...
fn historical_receipt_survives_later_revisions_and_stale_updates_are_rejected() {
    ...
    let ctx = store.local_context();
    let first_request =
        validate_submission(&request(&store, &registration, "u1", 0, "first")).unwrap();
...
fn unchanged_content_reuses_work_but_retains_each_submission_receipt() {
    ...
    let (first, _) = store
        .accept(
            &ctx,
            &validate_submission(&request(&store, &registration, "u1", 0, "same")).unwrap(),
        )
        .unwrap();
    let (same, _) = store
        .accept(
            &ctx,
            &validate_submission(&request(&store, &registration, "u2", 1, "same")).unwrap(),
        )
        .unwrap();
...
fn registration_namespaces_and_service_context_are_not_caller_controlled() {
    ...
    let ctx = store.local_context();
    assert!(store
        .register_source(&other.local_context(), SourceKind::Continue, "device")
        .is_err());
    ...
    let first = store
        ...
        .0;
    let different = store
        .register_source(&ctx, SourceKind::Continue, "device-2")
        .unwrap();
...
fn concurrent_retries_get_one_atomic_receipt() {
    ...
    assert!(receipts
        .iter()
        .all(|(receipt, _)| *receipt == receipts[0].0));
...
fn aba_content_does_not_attach_a_new_revision_to_an_old_running_job() {
    ...
    let first = store
        .accept(
            &ctx,
            &validate_submission(&request(&store, &registration, "u1", 0, "A")).unwrap(),
        )
        .unwrap()
        .0;
    db.conn()
    ...
    store
        .accept(
            &ctx,
            &validate_submission(&request(&store, &registration, "u2", 1, "B")).unwrap(),
        )
        .unwrap();
    let third = store
        .accept(
            &ctx,
            &validate_submission(&request(&store, &registration, "u3", 2, "A")).unwrap(),
        )
        .unwrap()
        .0;
...
fn a_mutated_validated_object_cannot_persist_unchecked_bytes() {
    ...
    assert_eq!(
        store
            .accept(&store.local_context(), &input)
            .unwrap_err()
            .code(),
        "invalid_input"
    );
