...
use aiks_core::{service::ServiceStore, storage::{SourceSessionRepo, StateDb}, team::{TeamError, TeamStore}};
...
fn company_binding_survives_restart_and_cannot_be_reassigned() {
    ...
    let (company, instance) = {
        ...
        assert!(matches!(TeamStore::bind(db.clone(), "other-corp", "synthetic-client"), Err(TeamError::ConfigInvalid)));
        assert!(matches!(TeamStore::bind(db.clone(), "synthetic-corp", "other-client"), Err(TeamError::ConfigInvalid)));
        assert!(ServiceStore::open(db.clone()).is_err(), "a company DB cannot become a local personal identity");
        let personal: i64 = db.conn().query_row("SELECT COUNT(*) FROM service_instance", [], |row| row.get(0)).unwrap();
        assert_eq!(personal, 0);
        (store.company_id().to_owned(), store.instance_id().to_owned())
    };
    let db = Arc::new(StateDb::open_exclusive(&path).unwrap());
...
fn personal_and_legacy_data_are_not_adopted_implicitly() {
    for initialized in [false, true] {
        ...
        if initialized { ServiceStore::open(db.clone()).unwrap(); }
        let id = SourceSessionRepo::new(&db).upsert("continue", "legacy", None, None, None, Some("KEEP"), None, None, None).unwrap();
        assert!(matches!(TeamStore::bind(db.clone(), "synthetic-corp", "synthetic-client"), Err(TeamError::ConfigInvalid)));
        let title: String = db.conn().query_row("SELECT title FROM source_session WHERE id=?1", [id], |row| row.get(0)).unwrap();
        assert_eq!(title, "KEEP");
        let companies: i64 = db.conn().query_row("SELECT COUNT(*) FROM team_company", [], |row| row.get(0)).unwrap();
...
fn binding_requires_writer_ownership_and_strict_identity() {
    ...
    assert!(matches!(TeamStore::bind(db, "synthetic-corp", "synthetic-client"), Err(TeamError::Storage)));
    let db = Arc::new(StateDb::open_exclusive(&root.path().join("invalid.db")).unwrap());
    for (corp, client) in [("", "client"), ("corp", ""), (" corp", "client"), ("corp", "client\n")] {
        assert!(matches!(TeamStore::bind(db.clone(), corp, client), Err(TeamError::ConfigInvalid)));
    }
    db.conn().execute_batch("DROP TABLE team_company").unwrap();
    assert!(matches!(TeamStore::bind(db, "synthetic-corp", "synthetic-client"), Err(TeamError::Storage)), "database errors must not turn into an ordinary authorization denial");
}
...
fn sharing_schema_enforces_read_only_and_same_company_ownership() {
    ...
    assert!(conn.execute("UPDATE document_share_grant SET permission='edit'", []).is_err());
    assert!(conn.execute("UPDATE document_share_grant SET company_id='other-company'", []).is_err());
    assert!(conn.execute("UPDATE document_share_grant SET target_user_id='missing-user'", []).is_err());
    assert!(conn.execute("UPDATE document_share_grant SET include_descendants=1", []).is_err());
    ...
    let version: i64 = conn.query_row("SELECT grant_version FROM team_knowledge_owner WHERE knowledge_id='knowledge'", [], |row| row.get(0)).unwrap();
    assert_eq!(version, 0);
}
