...
fn b14_rebuild_sync_index_preserves_knowledge() {
    ...
    repo.save_items(sid, None, &make_extraction("Test Knowledge"))
        .unwrap();

    // Verify knowledge exists
    let items_before: i64 = db
        .conn()
        .query_row("SELECT COUNT(*) FROM knowledge_item", [], |r| r.get(0))
        .unwrap();
    assert_eq!(
        items_before, 1,
        "Should have 1 knowledge item before rebuild"
    );
    ...
    let items_after: i64 = db
        .conn()
        .query_row("SELECT COUNT(*) FROM knowledge_item", [], |r| r.get(0))
        .unwrap();
    assert_eq!(
        items_after, 1,
        "Knowledge item must survive sync index rebuild"
    );
...
async fn b13_concurrency_limit_respected() {
    ...
    // Insert 3 sessions
    let ids: Vec<i64> = (0..3)
        .map(|i| insert_session(&db, "claude_code", &format!("session-{}", i)))
        .collect();

    let orchestrator = PipelineOrchestrator::new(db.clone());
    let run_ids: Vec<String> = ids
        .iter()
        .map(|id| orchestrator.enqueue(*id, Some("hash")).unwrap())
        .collect();
    ...
    assert_eq!(
        failed_count, 3,
        "All 3 jobs should have FAILED (no provider)"
    );
}
...
fn b17_malformed_json_is_not_skip() {
    ...
    // Valid skip response
    let valid_skip =
        r#"{"session_summary":"x","knowledge_score":0.3,"worth_extracting":false,"items":[]}"#;
    ...
    assert!(
        result.is_err(),
        "Malformed JSON should be Err, not silently skipped"
    );
...
async fn b04_target_hash_tracks_written_content() {
    ...
    assert_ne!(
        target.synced_hash, target.target_hash,
...
fn b09_enqueue_all_pending_creates_pipeline_runs() {
    ...
    assert_eq!(
        enqueued_again, 2,
        "READY sessions should not be re-enqueued"
    );
}
