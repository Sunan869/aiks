...
use aiks_core::{
    ...
    model::SourceKind,
    pipeline::{
        job_repo::PipelineJobRepo, EmbeddingConfig, PipelineJob, PipelineOrchestrator,
        PipelineWorker,
    },
    service::{
        adopt_local_state, query, AdoptionManifest, ServiceError, ServiceStore, SessionAdoption,
    },
...
impl Legacy {
    fn new(available: bool) -> Self {
        ...
        let run = PipelineOrchestrator::new(db.clone())
            .enqueue(id, Some("legacy-hash"))
            .unwrap();
        let job = PipelineJobRepo::new(&db)
            .enqueue(&PipelineJob {
                pipeline_run_id: run,
                session_id: id,
                session_external_id: "legacy-session".into(),
                source: "continue".into(),
                session_title: Some("Legacy title".into()),
                project_name: None,
            })
            .unwrap()
            .durable_job_id;
        {
            ...
            conn.execute(
                ...
                VALUES (?1,'legacy-session','continue','Legacy title','LEGACY_PROJECTION')",
                [id],
            )
            .unwrap();
        }
        ...
        let reg = store
            .register_source(&ctx, SourceKind::Continue, "legacy-device")
            .unwrap();
        ...
        let manifest = AdoptionManifest {
            ...
            sessions: vec![SessionAdoption {
                session_id: id,
                source: SourceKind::Continue,
                upstream_id: "legacy-session".into(),
                registration_id: reg,
                snapshot: available.then_some(input),
            }],
    ...
    fn preserved(&self) {
        ...
        let source: (i64, String, String) = conn
            .query_row(
                "SELECT id,source,external_session_id FROM source_session WHERE id=?1",
                [self.id],
        ...
        assert_eq!(
            source,
            (self.id, "continue".into(), "legacy-session".into())
        );
        ...
        assert_eq!(
            maps,
            ("old-doc".into(), "base-hash".into(), "remote-hash".into())
        );
        let raw: (String, String, String) = conn
            .query_row(
                "SELECT target_id,synced_hash,target_hash FROM sync_target WHERE session_id=?1",
                [self.id],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
            )
            .unwrap();
        assert_eq!(
            raw,
            ("raw-doc".into(), "raw-base".into(), "raw-remote".into())
        );
    }
}
...
fn mixed_manifest_identity_conflict_makes_no_partial_bindings() {
    ...
    let id = SourceSessionRepo::new(&f.db)
        .upsert(
            "continue", "another", None, None, None, None, None, None, None,
        )
        .unwrap();
...
fn failure_after_binding_rolls_back_and_retry_is_safe() {
    ...
    assert_eq!(f.count("pipeline_job"), 1);
    f.db.conn()
        .execute_batch("DROP TRIGGER break_snapshot")
        .unwrap();
...
async fn adopted_snapshot_runs_after_reopening_with_no_provider_or_source_file() {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            let status: String = db
                .conn()
                .query_row("SELECT status FROM pipeline_job WHERE id=?1", [&job], |r| {
                    r.get(0)
                })
                .unwrap();
            if status == "DONE" {
    ...
    let text: String = db
        .conn()
        .query_row("SELECT content FROM session_search_fts", [], |r| r.get(0))
        .unwrap();
