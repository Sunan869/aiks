use aiks_core::{
    ...
    model::{ContentBlock, MessageRole, SourceKind},
    providers::{
...
async fn each_cline_family_ui_fallback_keeps_user_assistant_and_thinking() {
    for source in [SourceKind::Cline, SourceKind::RooCode, SourceKind::KiloCode] {
        ...
        let task = root.path().join("tasks/任务一");
        std::fs::create_dir_all(&task).unwrap();
        std::fs::write(
            task.join("task_metadata.json"),
            json!({"title":"中文任务","cwd":"C:\\示例\\项目"}).to_string(),
        )
        .unwrap();
        ...
        assert_eq!(session.title.as_deref(), Some("中文任务"));
        assert_eq!(session.project_name.as_deref(), Some("项目"));
...
fn windows_junction_root_and_parent_cannot_escape_the_configured_store() {
    ...
    let parent_rejected = reader
        .read_json(Path::new("junction/private.json"))
        .is_err();
    std::fs::remove_dir(&link).unwrap();
    assert!(root_rejected && parent_rejected);
    assert_eq!(
        std::fs::read_to_string(outside.path().join("private.json")).unwrap(),
        "{}"
    );
}
...
fn explicit_root_relative_io_stays_platform_independent() {
    let root = tempfile::tempdir().unwrap();
    std::fs::write(root.path().join("会话.json"), "{}").unwrap();
