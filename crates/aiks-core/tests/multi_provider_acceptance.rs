...
async fn load(key: &str, root: &Path) -> NormalizedSession {
    ...
    let provider = registry
        .get(kind)
        .unwrap_or_else(|| panic!("unregistered source: {key}"));
    ...
    assert_eq!(
        compute_session_hash(&session),
        compute_session_hash(&second)
    );
...
async fn continue_imports_history_not_index_or_context_items() {
    let root = fixture(&[
        (
            "sessions/arbitrary.json",
            r#"{"sessionId":"s1","title":"Continue title","workspaceDirectory":"C:\\example\\中文","history":[{"message":{"role":"user","content":"CONTINUE_UNIQUE"},"contextItems":[{"content":"context only"}]},{"message":{"role":"assistant","content":[{"type":"text","text":"answer"}]}}]}"#,
        ),
...
async fn cline_family(key: &str, index: &str) {
    let root = fixture(&[
        (
            "tasks/t1/api_conversation_history.json",
            r#"[{"role":"user","content":[{"type":"text","text":"CLINE_FAMILY_UNIQUE"}]},{"role":"assistant","content":[{"type":"tool_use","id":"c1","name":"read_file","input":{"path":"example.txt"}}]},{"role":"user","content":[{"type":"tool_result","tool_use_id":"c1","content":"synthetic output"}]}]"#,
        ),
        (
            index,
            r#"[{"id":"t1","task":"Task title","ts":1000,"cwdOnTaskInitialization":"/example/p","workspace":"/example/p"}]"#,
        ),
    ]);
    ...
    assert!(s
        ...
        .iter()
        .flat_map(|m| &m.blocks)
        .any(|b| matches!(b, ContentBlock::ToolCall { id: Some(id), .. } if id == "c1")));
    assert!(s
        .messages
        .iter()
        .flat_map(|m| &m.blocks)
        .any(|b| matches!(b, ContentBlock::ToolResult { id: Some(id), .. } if id == "c1")));
}

#[tokio::test]
async fn cline_imports_api_history() {
    cline_family("cline", "state/taskHistory.json").await;
}
#[tokio::test]
async fn roo_code_imports_task_history() {
    cline_family("roo_code", "tasks/_index.json").await;
}
#[tokio::test]
async fn kilo_code_imports_without_cline_index() {
    cline_family("kilo_code", "tasks/t1/task_metadata.json").await;
}
...
async fn kimi_code_replays_wire_messages() {
    ...
    assert_eq!(
        text(&load("kimi_code", root.path()).await),
        "KIMI_UNIQUE\nanswer"
    );
}
...
async fn github_copilot_imports_real_cli_events() {
    ...
    assert_eq!(
        text(&load("github_copilot", root.path()).await),
        "COPILOT_UNIQUE\nanswer"
    );
}
...
async fn antigravity_imports_cli_transcript_not_token_stats() {
    ...
    assert_eq!(
        text(&load("antigravity", root.path()).await),
        "ANTIGRAVITY_UNIQUE\nanswer"
    );
}
...
async fn cursor_imports_sqlite_composer_messages_readonly() {
    ...
    conn.execute(
        ...
        rusqlite::params!["composerData:s1", composer.to_string()],
    )
    .unwrap();
    drop(conn);
    let before = std::fs::read(&path).unwrap();
    assert_eq!(
        text(&load("cursor", root.path()).await),
        "CURSOR_UNIQUE\nanswer"
    );
