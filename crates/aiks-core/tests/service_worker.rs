    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            let detail = PipelineRepo::new(db)
                .get_run_detail(&receipt.pipeline_run_id).unwrap().unwrap();
            if detail.status == "RAW_ONLY" { break; }
            assert_ne!(detail.status, "FAILED", "snapshot processing failed");
            tokio::time::sleep(Duration::from_millis(25)).await;
        }
    }).await.expect("snapshot must finish without its source files");
}
...
async fn real_continue_snapshot_remains_searchable_after_source_directory_is_deleted() {
    ...
    std::fs::write(source.path().join("sessions/example.json"), json!({
    ...
    let mut submission = fixture::submission(ctx.space_id(), ctx.instance_id(), &reg, "u1", 0, "unused");
    submission.session = session;
    let receipt = store.accept(&ctx, &validate_submission(&submission).unwrap()).unwrap().0;
    source.close().unwrap();
    drop(registry);
    ...
    let found = UnifiedSearchService::new(db.clone(), models).search(
        "OFFLINE_SNAPSHOT_UNIQUE", 10,
        UnifiedSearchFilter { corpora: vec![SearchCorpus::Session], ..Default::default() }
    ).await.unwrap();
    assert!(found.hits.iter().any(|hit| hit.entity_id == receipt.session_id));
...
async fn persisted_jobs_resume_after_reopening_and_do_not_consume_unadopted_legacy_jobs() {
    ...
    let receipt = {
        ...
        let ctx = store.local_context();
        let reg = store.register_source(&ctx, SourceKind::Continue, "device").unwrap();
        let submission = fixture::submission(ctx.space_id(), ctx.instance_id(), &reg, "u1", 0, "RESUME_UNIQUE");
        let receipt = store.accept(&ctx, &validate_submission(&submission).unwrap()).unwrap().0;
        ...
        PipelineJobRepo::new(&db).enqueue(&PipelineJob {
            pipeline_run_id: "legacy-run".into(), session_id: 900,
            session_external_id: "legacy".into(), source: "continue".into(),
            session_title: None, project_name: None,
        }).unwrap();
        receipt
    };
    let db = Arc::new(StateDb::open_exclusive(&path).unwrap());
    ...
    wait_raw(&db, &receipt).await;
    worker.shutdown(Duration::from_secs(2)).await.unwrap();
    let legacy: String = db.conn().query_row(
        "SELECT status FROM pipeline_job WHERE session_id=900", [], |row| row.get(0)
    ).unwrap();
    assert_eq!(legacy, "PENDING");
    drop(worker);
    drop(db);
    assert!(StateDb::open_exclusive(&path).is_ok(), "shutdown must release worker references");
}
...
fn run_input_is_its_immutable_revision_and_never_falls_back_to_a_provider_path() {
    ...
    let ctx = store.local_context();
    let reg = store.register_source(&ctx, SourceKind::Continue, "device").unwrap();
    let mut submission = fixture::submission(ctx.space_id(), ctx.instance_id(), &reg, "u1", 0, "first");
    submission.session.source_path = Some("/nonexistent/source.json".into());
    let first = store.accept(&ctx, &validate_submission(&submission).unwrap()).unwrap().0;
    submission.submission_id = "u2".into();
    submission.expected_revision = 1;
    submission.session.messages[0].blocks[0] = aiks_core::ContentBlock::Text { text: "second".into() };
    store.accept(&ctx, &validate_submission(&submission).unwrap()).unwrap();
