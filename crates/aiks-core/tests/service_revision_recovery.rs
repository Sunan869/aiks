    ...
    for retry in [false, true] {
        ...
        let (old, current) = {
            ...
            let mut request = fixture::submission(
                ctx.space_id(), ctx.instance_id(), &reg, "first", 0, "OLD_RETRY_TEXT",
            );
            let old = store.accept(&ctx, &validate_submission(&request).unwrap()).unwrap().0;
            ...
            PipelineRepo::new(&db).mark_started(&old.pipeline_run_id).unwrap();
            if retry {
                PipelineRepo::new(&db).mark_failed(&old.pipeline_run_id, "AI_EXTRACTED", "synthetic model failure").unwrap();
                jobs.mark_failed(&old.job_id, "synthetic model failure").unwrap();
            }
            ...
            let current = store.accept(&ctx, &validate_submission(&request).unwrap()).unwrap().0;
            if !retry {
                db.conn().execute(
        ...
        tokio::time::timeout(Duration::from_secs(10), async {
            loop {
                let status: String = db.conn().query_row(
                    "SELECT status FROM pipeline_job WHERE id=?1", [&current.job_id], |row| row.get(0),
                ).unwrap();
        ...
        let (status, attempts): (String, i64) = conn.query_row(
            "SELECT status,attempt FROM pipeline_job WHERE id=?1", [&old.job_id],
            |row| Ok((row.get(0)?, row.get(1)?)),
        ).unwrap();
        assert_eq!(status, "SUPERSEDED");
        assert_eq!(attempts, 1, "an obsolete snapshot must not make another model attempt");
        let status: String = conn.query_row(
            "SELECT status FROM pipeline_run WHERE id=?1", [&old.pipeline_run_id], |row| row.get(0),
        ).unwrap();
        assert_eq!(status, "SUPERSEDED");
        let (indexed, completed): (u32, u32) = conn.query_row(
            "SELECT indexed_revision,completed_revision FROM service_derived_state", [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        ).unwrap();
        assert_eq!((indexed, completed), (2, 2));
        let content: String = conn.query_row(
            "SELECT content FROM session_search_fts", [], |row| row.get(0),
        ).unwrap();
...
fn superseded_status_has_stable_wire_identity_and_is_not_ready_or_failed() {
    ...
    assert_eq!(serde_json::from_str::<PipelineStage>("\"SUPERSEDED\"").unwrap(), status);
}
