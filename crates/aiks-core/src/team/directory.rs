...
impl TeamStore {
    pub fn publish_directory(&self, input: DirectorySnapshot, now: u64) -> Result<u64, TeamError> {
        ...
        let generation = current.checked_add(1).filter(|g| *g <= i64::MAX as u64)
        ...
        for user in &input.users {
            ...
            let id = match existing {
                Some((id, external, active)) => {
                    if external != user.external_user_id { return Err(TeamError::Conflict); }
                    if active && !user.active { deactivate(&tx, company, &id, now)?; }
                ...
                None => {
                    ...
                    if occupied { return Err(TeamError::Conflict); }
        ...
        let missing = {
            ...
            let rows = stmt.query_map(params![company,generation], |row| row.get::<_,String>(0))?;
            rows.collect::<Result<Vec<_>,_>>()?
        };
        for id in missing { deactivate(&tx, company, &id, now)?; }
        ...
        for org in &input.orgs {
            let existing: Option<String> = tx.query_row(
                ...
                params![company,org.id], |row| row.get(0),
            ).optional()?;
            let id = match existing {
                Some(id) => id,
                None => {
        ...
        tx.execute("UPDATE team_company SET directory_generation=?2 WHERE id=?1",params![company,generation])?;
        ...
        tx.execute("DELETE FROM team_org_snapshot WHERE company_id=?1 AND generation<?2",
    ...
    pub fn directory_generation(&self) -> Result<u64, TeamError> {
        Ok(self.db.conn().query_row("SELECT directory_generation FROM team_company WHERE id=?1",
            [self.company_id()], |row| row.get(0))?)
    }
    ...
    pub fn is_org_member(&self, user: &str, org: &str, descendants: bool) -> Result<bool, TeamError> {
        Ok(self.db.conn().query_row(
    ...
    pub fn mark_member_inactive(&self, user: &str, now: u64) -> Result<(), TeamError> {
        if now > i64::MAX as u64 { return Err(TeamError::InvalidInput); }
        let mut conn = self.db.conn();
        let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
...
pub(super) fn directory_fresh(conn: &Connection, company: &str, now: u64, max_age: u64) -> Result<bool,TeamError> {
    if max_age == 0 || max_age > 3600 { return Ok(false); }
    let observed: Option<u64> = conn.query_row(
...
fn deactivate(tx: &Transaction<'_>, company: &str, user: &str, now: u64) -> Result<(),TeamError> {
    let active: bool = tx.query_row("SELECT active FROM team_user WHERE company_id=?1 AND id=?2",
        params![company,user], |row| row.get(0)).optional()?.ok_or(TeamError::NotFound)?;
    ...
    tx.execute("UPDATE team_user SET active=0 WHERE company_id=?1 AND id=?2",params![company,user])?;
    tx.execute("INSERT INTO team_audit_event(id,company_id,actor_user_id,action,occurred_at)
        VALUES (?1,?2,?3,'member_inactive',?4)",params![Uuid::new_v4().to_string(),company,user,now])?;
    Ok(())
}
