...
use super::{ingestion::accept_in_tx, validate_submission, validation::validate_identifier,
    LocalContext, ServiceError, SnapshotReceipt, SnapshotSubmission, ValidatedSnapshot};
...
pub fn adopt_local_state(
    ...
    if !db.has_exclusive_lease() { return Err(ServiceError::Unavailable); }
    if manifest.service_instance_id != context.instance_id() { return Err(ServiceError::Unauthorized); }
    ...
    let owned: bool = tx.query_row(
        ...
        params![context.instance_id(),context.principal_id(),context.space_id()],|r|r.get(0))?;
    if !owned { return Err(ServiceError::Unauthorized); }
    let running: bool = tx.query_row("SELECT EXISTS(SELECT 1 FROM pipeline_job WHERE status='RUNNING')",[],|r|r.get(0))?;
    if running { return Err(ServiceError::Conflict); }
    let mut report = AdoptionReport::default();
    for item in &manifest.sessions {
        if !identity_matches(&tx,context,item)? { report.conflict.push(item.session_id); }
    }
    // A conflicting manifest is all-or-nothing, even if earlier entries matched.
    if !report.conflict.is_empty() { return Ok(report); }
    for id in &manifest.knowledge_ids { verify_knowledge(&tx,context,id,manifest)?; }
    ...
    for (item,snapshot) in manifest.sessions.iter().zip(&checked) {
        ...
        if let Some(snapshot)=snapshot {
            ...
            let revision:u32=tx.query_row("SELECT current_revision FROM service_session_binding WHERE session_id=?1",[item.session_id],|r|r.get(0))?;
            if revision==0 { report.source_unavailable.push(item.session_id); }
            else { report.snapshot_ready.push(item.session_id); }
        }
...
fn validate_manifest(ctx:&LocalContext, manifest:&AdoptionManifest)->Result<Vec<Option<ValidatedSnapshot>>,ServiceError> {
    if manifest.sessions.len()>256 || manifest.knowledge_ids.len()>1024 { return Err(ServiceError::TooLarge); }
    let mut ids=HashSet::new();
    let mut total=0_usize;
    let mut checked=Vec::with_capacity(manifest.sessions.len());
    for item in &manifest.sessions {
        if item.session_id<=0 || !ids.insert(item.session_id) { return Err(ServiceError::InvalidInput); }
        validate_identifier(&item.upstream_id)?;
        validate_identifier(&item.registration_id)?;
        let snapshot=match &item.snapshot {
            Some(input)=>{
                if input.service_instance_id!=ctx.instance_id() || input.space_id!=ctx.space_id()
                    || input.source_registration_id!=item.registration_id
                    || input.session.source!=item.source || input.session.external_session_id!=item.upstream_id
                    || input.expected_revision!=0 { return Err(ServiceError::InvalidInput); }
                let checked=validate_submission(input)?;
                total=total.checked_add(checked.canonical_json.len()).ok_or(ServiceError::TooLarge)?;
                if total>32*1024*1024 { return Err(ServiceError::TooLarge); }
                Some(checked)
            }
            None=>None,
        };
    ...
    for id in &manifest.knowledge_ids {
        ...
        if !knowledge_ids.insert(id) { return Err(ServiceError::InvalidInput); }
    }
    Ok(checked)
}

fn identity_matches(tx:&Transaction<'_>,ctx:&LocalContext,item:&SessionAdoption)->Result<bool,ServiceError> {
    let registered:bool=tx.query_row(
        ...
        params![item.registration_id,ctx.principal_id(),ctx.space_id(),item.source.as_str()],|r|r.get(0))?;
    if !registered { return Err(ServiceError::NotFound); }
    let row:Option<(String,String)>=tx.query_row("SELECT source,external_session_id FROM source_session WHERE id=?1",
        [item.session_id],|r|Ok((r.get(0)?,r.get(1)?))).optional()?;
    let Some((source,upstream))=row else { return Err(ServiceError::NotFound); };
    if source!=item.source.as_str() || upstream!=item.upstream_id { return Ok(false); }
...
fn verify_knowledge(tx:&Transaction<'_>,ctx:&LocalContext,id:&str,manifest:&AdoptionManifest)->Result<(),ServiceError> {
    let row:Option<(Option<i64>,Option<String>,Option<String>)>=tx.query_row(
    ...
    if owner.as_deref().is_some_and(|v|v!=ctx.principal_id()) || space.as_deref().is_some_and(|v|v!=ctx.space_id()) {
    ...
    if let Some(source)=source {
        if !manifest.sessions.iter().any(|s|s.session_id==source) {
            ...
            if !owned { return Err(ServiceError::Conflict); }
        }
    }
    Ok(())
}

fn record_legacy_jobs(tx:&Transaction<'_>,session:i64,report:&mut AdoptionReport)->Result<(),ServiceError> {
    ...
    let rows=stmt.query_map([session],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?)))?;
    for row in rows {
        let (id,status)=row?;
        if report.retained_legacy_jobs.len()+report.retired_legacy_jobs.len()>=1000 { return Err(ServiceError::TooLarge); }
        if status=="SUPERSEDED" {report.retired_legacy_jobs.push(id);} else {report.retained_legacy_jobs.push(id);}
    }
    Ok(())
}
