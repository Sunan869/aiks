...
pub(crate) fn read(
    ...
    .ok_or_else(|| anyhow::anyhow!("Kimi session directory missing"))?;
    let mut meta = Value::Null;
    let state = dir.join("state.json");
    if io.exists(&state)? {
        meta = io.read_json(&state)?;
    }
    let upstream = string(&meta, "id")
        .or_else(|| dir.file_name().and_then(|n| n.to_str()).map(str::to_owned))
        .ok_or_else(|| anyhow::anyhow!("Kimi session ID missing"))?;
    let mut s = session(
        SourceKind::KimiCode,
