...
fn origin(m: &NormalizedMessage) -> Option<&str> {
    m.metadata
        .get("origin")
...
fn anchor(m: &NormalizedMessage) -> bool {
    ...
    match origin(m) {
        ...
        Some("skill_activation" | "plugin_command") => {
            m.metadata["origin"]["trigger"].as_str() == Some("user-slash")
        }
        _ => false,
    }
}
...
impl Fold {
        ...
        if let Some(index) = self.open.take() {
            if let Some(m) = self.history.get_mut(index) {
                if !self.pending.is_empty() {
                    let mut ids: Vec<_> = self.pending.iter().cloned().collect();
                    ids.sort();
                    m.metadata
                        .insert("interrupted_tool_calls".into(), json!(ids));
                }
    ...
    fn undo(&mut self, count: usize) {
        ...
        for i in (0..self.history.len()).rev() {
            ...
            if origin(m) == Some("compaction_summary") {
                return;
            }
            if anchor(m) {
                remaining -= 1;
                let mut start = i;
                while start > 0
                    && origin(&self.history[start - 1]) == Some("injection")
                    && self.history[start - 1].metadata["origin"]["ownerPromptId"].as_str()
                        == Some(&m.external_id)
                {
                    start -= 1;
                }
    ...
    fn compact(&mut self, record: &Value, index: usize) -> Result<()> {
        let count = record
            ...
            .ok_or_else(|| anyhow::anyhow!("Kimi compaction boundary missing"))?
            as usize;
        ensure!(
            count <= self.history.len(),
            "Kimi compaction boundary exceeds recorded history"
        );
        let summary = record
            .get("contextSummary")
            .or_else(|| record.get("summary"))
            .ok_or_else(|| anyhow::anyhow!("Kimi compaction summary missing"))?;
        let mut m = if summary.is_object() {
            standard_message(summary, index)
        } else {
            message(
                format!("kimi-compaction-{index}"),
                MessageRole::User,
                blocks(summary),
            )
        };
            ...
            !m.blocks.is_empty(),
            "Kimi compaction summary has no recorded content"
        );
        m.created_at = record.get("time").and_then(timestamp);
        m.metadata
            .insert("origin".into(), json!({"kind":"compaction_summary"}));
        let kept = record.get("keptUserMessageCount").and_then(Value::as_u64);
        let legacy = record
            .get("legacyTail")
            .and_then(Value::as_bool)
            .unwrap_or(kept.is_none());
        if legacy {
            ...
            let kept =
                kept.ok_or_else(|| anyhow::anyhow!("Kimi kept prompt count missing"))? as usize;
            let head = record["keptHeadUserMessageCount"].as_u64().unwrap_or(0) as usize;
            ensure!(
                head <= kept && kept <= users.len(),
                "Kimi compaction prompt selection invalid"
            );
            ...
            for user in &mut selected {
                user.metadata
                    .insert("compaction_original_user_text".into(), Value::Bool(true));
            }
            selected.push(m);
...
pub(crate) fn replay(
    ...
    let report = io.for_each_jsonl(relative, |index, record| {
        let kind = record["type"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Kimi wire event type missing"))?;
        let stamp = record.get("time").and_then(timestamp);
        match kind {
            "profile.bind" | "llm.request" => {
                if let Some(model) =
                    string(&record, "modelAlias").or_else(|| string(&record, "model"))
                {
                    f.model = Some(model);
                }
            }
            ...
            "context.append_message" => {
                let body = record
                    .get("message")
                    .ok_or_else(|| anyhow::anyhow!("Kimi append missing message"))?;
                let mut m = standard_message(body, index);
                m.created_at = stamp;
                if let Some(origin) = body.get("origin") {
                    m.metadata.insert("origin".into(), origin.clone());
                }
                if let Some(calls) = body.get("toolCalls").and_then(Value::as_array) {
                    m.blocks.extend(calls.iter().flat_map(blocks));
                }
            ...
            "context.append_loop_event" => {
                let event = record
                    .get("event")
                    .ok_or_else(|| anyhow::anyhow!("Kimi loop event missing"))?;
                match event["type"].as_str().unwrap_or("") {
                    "step.begin" => {
                        f.close();
                        let mut m = message(
                            format!("kimi-step-{index}"),
                            MessageRole::Assistant,
                            Vec::new(),
                        );
                        m.created_at = stamp;
                        m.model = f.model.clone();
                        f.open = Some(f.history.len());
                        f.history.push(m);
                    }
                    "step.end" => f.close(),
                    "content.part" => {
                        let open = f
                            .open
                            .ok_or_else(|| anyhow::anyhow!("Kimi content outside recorded step"))?;
                        f.history[open].blocks.extend(blocks(&event["part"]));
                    }
                    "tool.call" => {
                        let id = string(event, "toolCallId")
                            .ok_or_else(|| anyhow::anyhow!("Kimi tool call ID missing"))?;
                        let open = f.open.ok_or_else(|| {
                            anyhow::anyhow!("Kimi tool call outside recorded step")
                        })?;
                        ensure!(
                            f.pending.insert(id.clone()),
                            "duplicate pending Kimi tool call"
                        );
                        f.history[open].blocks.push(ContentBlock::ToolCall {
                            id: Some(id),
                            name: string(event, "name").unwrap_or_else(|| "unknown".into()),
                            input: event.get("args").cloned().unwrap_or(Value::Null),
                        });
                    }
                    "tool.result" => {
                        let id = string(event, "toolCallId")
                            .ok_or_else(|| anyhow::anyhow!("Kimi tool result ID missing"))?;
                        let known = f.pending.remove(&id);
                        let result = &event["result"];
                        let mut m = message(
                            format!("kimi-result-{index}"),
                            MessageRole::Tool,
                        ...
                        f.history.push(m);
                        if f.pending.is_empty() {
                            f.history.append(&mut f.deferred);
                        }
                    }
                    _ => bail!("unsupported Kimi context loop event; previous import retained"),
                }
            }
            _ if kind.starts_with("context.") => {
                bail!("unsupported Kimi context mutation; previous import retained")
            }
    ...
    if f.open.is_some() {
        session
            .metadata
            .insert("open_step".into(), Value::Bool(true));
    }
    f.close();
    session.messages = f.history;
    Ok(())
}
