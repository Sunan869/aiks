...
pub(crate) fn read(
    ...
    let id =
        string(&value, "sessionId").ok_or_else(|| anyhow::anyhow!("Continue sessionId missing"))?;
    let history = value
        .get("history")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow::anyhow!("Continue history array missing"))?;
    let mut s = session(
        SourceKind::Continue,
        id,
    ...
    if !metadata_only {
        for (index, item) in history.iter().enumerate() {
            let body = item
                .get("message")
                .ok_or_else(|| anyhow::anyhow!("Continue history entry missing message"))?;
            ...
            if let Some(states) = item.get("toolCallStates").and_then(Value::as_array) {
                ...
                !m.blocks.is_empty(),
                "Continue message has no supported content; retaining previous import"
            );
            s.messages.push(m);
        }
    }
