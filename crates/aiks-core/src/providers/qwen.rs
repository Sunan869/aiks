...
pub(crate) fn read(io: &ScopedReader, relative: &Path, metadata_only: bool) -> Result<Vec<NormalizedSession>> {
    ...
    let report = io.for_each_jsonl(relative, |index, event| {
        let id = string(&event, "sessionId").ok_or_else(|| anyhow::anyhow!("Qwen record missing sessionId"))?;
        let s = sessions.entry(id.clone()).or_insert_with(|| session(SourceKind::QwenCode, id, &path, "qwen-jsonl"));
        if let Some(cwd) = string(&event, "cwd") { s.project_path = Some(cwd); }
        if let Some(title) = string(&event, "title") { s.title = Some(title); }
        if let Some(model) = string(&event, "model") { s.model = Some(model); }
        ...
        let Some(body) = event.get("message") else { return Ok(()); };
        if !metadata_only || s.title.is_none() {
            ...
            if !m.blocks.is_empty() {
                s.messages.push(m);
                if metadata_only { complete(s); s.messages.clear(); }
            }
        }
        Ok(())
    })?;
    ensure!(report.complete, "Qwen transcript is incomplete; previous import must be retained");
    ensure!(!sessions.is_empty(), "Qwen transcript has no session identity");
    Ok(sessions.into_values().map(|mut s| { complete(&mut s); s }).collect())
}
