...
pub(crate) fn read(
    ...
    let report = io.for_each_jsonl(relative, |index, event| {
        ...
        report.complete,
        "Cursor Agent transcript incomplete; retaining previous import"
    );
    ensure!(
        !s.messages.is_empty(),
        "Cursor Agent transcript contains no readable messages"
    );
    complete(&mut s);
    Ok(vec![s])
}
