...
impl Default for DiscoveryReport {
    fn default() -> Self {
        Self { sessions: Vec::new(), complete: true, diagnostics: Vec::new(), covered_paths: Vec::new() }
    }
}
impl DiscoveryReport {
    pub fn incomplete(&mut self, code: &str, store_index: usize) {
        self.complete = false;
        if let Some(d) = self.diagnostics.iter_mut().find(|d| d.code == code && d.store_index == store_index) {
            d.count += 1;
        } else {
            self.diagnostics.push(DiscoveryDiagnostic { code: code.to_owned(), store_index, count: 1 });
        }
    }
}
