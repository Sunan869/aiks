...
pub fn external_config(config: &Config, source: SourceKind) -> Option<&ExternalProviderConfig> {
    ...
    Some(match source {
        SourceKind::Antigravity => &p.antigravity,
        SourceKind::Cursor => &p.cursor,
        SourceKind::CursorAgent => &p.cursor_agent,
        SourceKind::Cline => &p.cline,
        SourceKind::RooCode => &p.roo_code,
        SourceKind::KiloCode => &p.kilo_code,
        SourceKind::GithubCopilot => &p.github_copilot,
        SourceKind::KimiCode => &p.kimi_code,
        SourceKind::QwenCode => &p.qwen_code,
        SourceKind::Continue => &p.continue_dev,
        SourceKind::Aider => &p.aider,
        _ => return None,
    })
}
...
pub fn descriptors(
    config: &Config,
    health: &[(SourceKind, ProviderHealth)],
    ...
    ALL_SOURCES
        ...
        .map(|&source| {
            ...
            let status = if !enabled {
                ...
                match h {
                    Some(ProviderHealth::Ok) => "ok",
                    Some(ProviderHealth::Unsupported { .. }) => "unsupported",
                    Some(ProviderHealth::NotConfigured) => "not_configured",
                    Some(ProviderHealth::Error { .. }) => "error",
                    Some(ProviderHealth::NotFound { .. }) => "not_found",
                    None => "unknown",
                }
            };
            ProviderDescriptor {
                ...
                paths,
                status: status.into(),
                message: h.map_or("", ProviderHealth::message).into(),
            }
        })
        .collect()
}
