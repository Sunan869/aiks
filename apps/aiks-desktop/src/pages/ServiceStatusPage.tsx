...
export default function ServiceStatusPage(){
  ...
  return <main className="min-h-screen bg-gray-50 p-6 text-gray-800 space-y-5">
    <DataStorageSetupGate onRequired={storageRequired}/><header className="flex items-center justify-between"><div><h1 className="text-2xl font-semibold">AIKS · 本地知识服务</h1><p className="text-sm text-gray-500 mt-1">独立服务模式 · 数据只在本机 · 思源通过内部适配器访问</p></div><span className="rounded-full border bg-white px-4 py-2 text-sm">{ready?"服务已连接":status?.phase??"正在启动"}</span></header>
    <div className="rounded-lg border border-blue-100 bg-blue-50 p-4 text-sm leading-6">这是独立的本地空间，旧个人知识库不会自动迁移或上传。模型配置位于数据目录的 <code>service-local/config/models.toml</code>，修改后重启。AI 提炼：{status?.capabilities?.ai_assist?"已启用":"未启用"}；语义搜索：{status?.capabilities?.semantic_search?"已启用":"未启用"}。未启用 AI 时仍可采集和检索会话。此页面不开放思源全库接口。</div>
    ...
    <section className={card}><h2 className="font-semibold">选择本地来源采集</h2><p className="mt-1 text-sm text-gray-500">只读取勾选来源，每次最多 100 条；只提交完整会话。上传前执行规则脱敏，但不能保证识别所有敏感内容。来源目录沿用原有配置。</p>
