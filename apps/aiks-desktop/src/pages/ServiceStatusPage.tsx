// Personal and optional team workspaces share one shell; selecting team never shuts down local Service.
import {useEffect,useState,useSyncExternalStore} from "react";
import {Activity,BookOpen,Building2,MessageSquare,Settings,SlidersHorizontal,UserRound} from "lucide-react";
...
import TeamConnection from "./service/TeamConnection";
...
export default function ServiceStatusPage(){
  ...
  useEffect(()=>{let active=true,inflight=false;const load=async()=>{if(inflight)return;inflight=true;
    try{const value=await serviceApi.status();if(active)setStatus(value);}catch{if(active)setStatus(s=>({...s,mode:"service_local",phase:"unavailable"}));}finally{inflight=false;}};
  ...
  const ready=status?.phase==="ready";const sources=state.preferences?.selected_sources??[];
  return <div className="flex h-screen min-h-0 flex-col bg-slate-50 text-slate-800">
    <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b bg-white px-5">
      <div className="flex items-center gap-3"><strong className="text-lg text-blue-600">AIKS</strong><span className="hidden text-xs text-slate-500 md:inline">{workspace==="personal"?"本地知识服务 · 个人独立部署版":"团队知识 · 显式连接"}</span></div>
      <div className="flex rounded-lg bg-slate-100 p-1" aria-label="知识空间切换"><button className={`flex items-center gap-1 rounded-md px-3 py-1.5 text-xs ${workspace==="personal"?"bg-white font-medium text-blue-700 shadow-sm":"text-slate-500"}`} onClick={()=>setWorkspace("personal")}><UserRound size={14}/>个人空间</button><button className={`flex items-center gap-1 rounded-md px-3 py-1.5 text-xs ${workspace==="team"?"bg-white font-medium text-blue-700 shadow-sm":"text-slate-500"}`} onClick={()=>setWorkspace("team")}><Building2 size={14}/>团队空间</button></div>
      {workspace==="personal"?<span className={`rounded-full px-3 py-1 text-xs ${ready?"bg-emerald-50 text-emerald-700":"bg-amber-50 text-amber-700"}`}>{ready?"服务已连接":status?.phase==="starting"||!status?"服务正在启动":"服务暂不可用"}</span>:<span className="rounded-full bg-blue-50 px-3 py-1 text-xs text-blue-700">可选团队连接</span>}
    </header>
    {workspace==="team"?<div className="min-h-0 flex-1"><TeamConnection/></div>:<div className="flex min-h-0 flex-1"><nav aria-label="主导航" className="flex w-48 shrink-0 flex-col border-r bg-white p-3"><div className="mb-4 px-3 py-3"><p className="text-sm font-semibold">我的个人空间</p><p className="mt-1 text-xs text-slate-400">本机存储 · 不自动迁移旧库</p></div>{nav.map(item=><button key={item.key} aria-current={state.page===item.key?"page":undefined} onClick={()=>controller.navigate(item.key)} className={`mb-1 flex items-center gap-3 rounded-lg px-3 py-3 text-left text-sm ${state.page===item.key?"bg-blue-50 font-medium text-blue-700":"text-slate-600 hover:bg-slate-50"}`}><item.icon size={17}/>{item.label}</button>)}<div className="mt-auto border-t px-3 pt-4 text-xs leading-6 text-slate-400"><p>AI 提炼：{status?.capabilities?.ai_assist?"已启用":"未启用"}</p><p>语义检索：{status?.capabilities?.semantic_search?"已启用":"未启用"}</p><p className="mt-2">未配置模型也可以阅读和关键词检索。</p></div></nav>
