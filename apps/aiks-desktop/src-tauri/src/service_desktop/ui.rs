...
use crate::service_client::{
    browse::LocalSessionBrowser,
    preferences::{Onboarding, SourceScope, UiPreferences},
};
...
impl ServiceDesktop {
    ...
    pub async fn ui_preferences(&self) -> Result<UiPreferences, String> {
        ...
        tokio::task::spawn_blocking(move || store.ui_preferences())
            .await
            .map_err(|_| "collector_storage_unavailable")?
    ...
    pub async fn finish_onboarding(&self, skipped: bool) -> Result<UiPreferences, String> {
        ...
        tokio::task::spawn_blocking(move || {
            store.finish_onboarding(if skipped {
                Onboarding::Skipped
            } else {
    ...
    pub async fn save_sources(&self, sources: Vec<String>) -> Result<UiPreferences, String> {
        ...
        let store = self.local_store().await?;
        tokio::task::spawn_blocking(move || store.save_selected_sources(sources))
            .await
            .map_err(|_| "collector_storage_unavailable")?
            .map_err(|e| e.to_string())
    }
    ...
    pub(super) fn source_key(&self, source: aiks_core::SourceKind) -> Result<String, String> {
        let paths = descriptors(&self.provider_config, &[])
            .into_iter()
    ...
    async fn source_scope(
        &self,
        source: aiks_core::SourceKind,
    ) -> Result<Option<SourceScope>, String> {
    ...
    pub async fn scan_sessions(&self, key: String) -> Result<Value, String> {
        ...
        let browser =
            tokio::time::timeout(Duration::from_secs(30), LocalSessionBrowser::scan(provider))
                .await
    ...
    pub async fn browse_sessions(
        ...
        let excluded = if let Some(scope) = scope {
            let store = self.local_store().await?;
            tokio::task::spawn_blocking(move || store.excluded(&scope))
                .await
                .map_err(|_| "collector_storage_unavailable")?
                .map_err(|e| e.to_string())?
        } else {
    ...
    pub async fn preview_session(&self, key: String, row: String) -> Result<Value, String> {
        ...
        let provider = self
            ...
            .ok_or("source_disabled_or_unavailable")?;
        let preview =
            tokio::time::timeout(Duration::from_secs(15), browser.preview(provider, &row))
                .await
                .map_err(|_| "scan_timeout")?
                .map_err(|e| e.to_string())?;
        serde_json::to_value(preview).map_err(|_| "invalid_session_preview".into())
    }
    pub async fn exclude_sessions(
        &self,
        key: String,
        rows: Vec<String>,
        excluded: bool,
        ...
        let ids = rows
            .iter()
            .map(|key| browser.resolve(key).map(|s| s.external_session_id.clone()))
            .collect::<Result<Vec<_>, _>>()
            .map_err(|_| "scan_required")?;
        let scope = self
            .source_scope(source)
            .await?
        ...
        let store = self.local_store().await?;
        let change =
            tokio::task::spawn_blocking(move || store.set_excluded(&scope, &ids, excluded))
                .await
                .map_err(|_| "collector_storage_unavailable")?
                .map_err(|e| e.to_string())?;
