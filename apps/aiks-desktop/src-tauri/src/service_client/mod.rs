...
pub enum ClientError {
    InvalidInput, WrongInstance, Unauthorized, NotFound, Conflict,
    Retryable, Busy, TooLarge, Storage, InvalidResponse,
}
impl ClientError {
    pub fn code(self) -> &'static str {
        match self {
            Self::InvalidInput=>"invalid_input",Self::WrongInstance=>"wrong_instance",
            Self::Unauthorized=>"unauthorized",Self::NotFound=>"not_found",Self::Conflict=>"conflict",
            Self::Retryable=>"retryable",Self::Busy=>"previous_upload_pending",Self::TooLarge=>"too_large",
            Self::Storage=>"collector_storage_unavailable",Self::InvalidResponse=>"invalid_service_response",
        }
    }
}
...
impl From<rusqlite::Error> for ClientError { fn from(_:rusqlite::Error)->Self { Self::Storage } }
...
impl PendingSubmission {
    pub fn new(submission:SnapshotSubmission)->ClientResult<Self> {
        ...
        Ok(Self {submission,body,request_hash:validated.request_hash,content_hash:validated.content_hash})
    }
    pub fn submission(&self)->&SnapshotSubmission { &self.submission }
    pub fn payload_hash(&self)->&str { &self.request_hash }
