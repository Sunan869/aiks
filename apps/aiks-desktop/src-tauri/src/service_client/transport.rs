...
impl ServiceConnection {
    pub fn local(url: &str, instance: &str, space: &str, token: &str) -> ClientResult<Self> {
        ...
        if !allowed
            || base.scheme() != "http"
            || base.port_or_known_default().is_none_or(|v| v == 0)
...
impl ServiceClient {
    ...
    pub async fn register_source(&self, source: SourceKind, key: &str) -> ClientResult<String> {
        ...
        #[derive(Deserialize)]
        struct Registration { source_registration_id: String }
    ...
    pub async fn get_receipt(&self, id: &str) -> ClientResult<SnapshotReceipt> { self.read(&["receipts", id]).await }
    ...
    pub async fn session(&self, id: &str) -> ClientResult<Value> { self.read(&["sessions", id]).await }
    pub async fn knowledge_list(&self) -> ClientResult<Value> { self.read(&["knowledge"]).await }
