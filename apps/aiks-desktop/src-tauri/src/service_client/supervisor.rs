...
impl ReadyHandshake {
    pub fn parse(frame: &str, expected: Option<&str>) -> ClientResult<Self> {
        ...
        if value.api_version != 1
            || uuid::Uuid::parse_str(&value.boot_nonce).is_err()
            || !super::valid_id(&value.instance_id)
            || !super::valid_id(&value.space_id)
        {
            return Err(ClientError::InvalidResponse);
        }
        ...
        if expected.is_some_and(|id| id != value.instance_id) {
...
impl OwnedService {
    pub async fn start(binary: &Path, config: &Path, expected: Option<&str>) -> ClientResult<Self> {
        for path in [binary, config] {
            ...
            if !path.is_absolute() || !meta.is_file() || meta.file_type().is_symlink() {
        ...
        let result = tokio::time::timeout(Duration::from_secs(20), async {
            input
                .write_all(
                    format!(
                        "{}\n",
                        json!({"token":token,"owner_control":true,"owner_lifetime":true})
                    )
                    .as_bytes(),
                )
                .await
            ...
            let connection = ServiceConnection::local(
                &format!("http://{}", ready.address),
                &ready.instance_id,
                &ready.space_id,
                &token,
            )?;
    ...
    pub async fn shutdown(&mut self) -> ClientResult<bool> {
        ...
        if let Some(mut input) = self.control.take() {
            ...
            let _ =
                tokio::time::timeout(Duration::from_secs(2), input.write_all(command.as_bytes()))
                    .await;
        }
...
pub fn binary_path(resource_dir: &Path) -> ClientResult<PathBuf> {
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if dev.is_file() {
            return dev.canonicalize().map_err(|_| ClientError::InvalidInput);
        }
    }
