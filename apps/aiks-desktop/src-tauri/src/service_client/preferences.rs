...
impl SourceScope {
    pub fn new(instance: &str, space: &str, source: SourceKind, source_key: &str) -> ClientResult<Self> {
        if !valid_id(instance) || !valid_id(space) || !valid_id(source_key) || source_key.len() > 128 {
            return Err(ClientError::InvalidInput);
        }
        Ok(Self { instance: instance.into(), space: space.into(), source, source_key: source_key.into() })
    }
...
impl CollectorOutbox {
    pub fn ui_preferences(&self) -> ClientResult<UiPreferences> {
        ...
        let value: Option<String> = conn.query_row(
            "SELECT value FROM collector_meta WHERE key='ui/preferences-v1'", [], |r| r.get(0),
        ).optional()?;
        value.map(|text| serde_json::from_str(&text).map_err(|_| ClientError::Storage))
            .unwrap_or_else(|| Ok(UiPreferences::default()))
    }
    pub fn finish_onboarding(&self, state: Onboarding) -> ClientResult<UiPreferences> {
        if state == Onboarding::Unseen { return Err(ClientError::InvalidInput); }
    ...
    pub fn save_selected_sources(&self, sources: Vec<String>) -> ClientResult<UiPreferences> {
        if sources.len() > 16 || sources.iter().any(|s| {
            SourceKind::from_str(s).is_none_or(|kind| kind.as_str() != s)
        }) { return Err(ClientError::InvalidInput); }
    ...
    fn change_ui(&self, change: impl FnOnce(&mut UiPreferences)) -> ClientResult<UiPreferences> {
        ...
        tx.execute("INSERT INTO collector_meta(key,value) VALUES('ui/preferences-v1',?1)
    ...
    pub fn excluded(&self, scope: &SourceScope) -> ClientResult<Vec<String>> {
        ...
        let value: Option<String> = self.conn()?.query_row(
            "SELECT value FROM collector_meta WHERE key=?1", [key], |r| r.get(0),
        ).optional()?;
        decode_rules(value)
    }
    ...
    pub fn set_excluded(&self, scope: &SourceScope, ids: &[String], excluded: bool) -> ClientResult<ExclusionChange> {
        ...
        let mut conn = self.conn()?;
        let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let text: Option<String> = tx.query_row("SELECT value FROM collector_meta WHERE key=?1", [&rules_key], |r| r.get(0)).optional()?;
        let mut rules = decode_rules(text)?;
        let registration: Option<String> = tx.query_row("SELECT value FROM collector_meta WHERE key=?1", [registration_key], |r| r.get(0)).optional()?;
        let mut result = ExclusionChange { excluded_count: 0, paused: 0, already_received: 0 };
        let ids = ids.iter().collect::<std::collections::BTreeSet<_>>();
        for id in ids {
            if excluded {
                if !rules.contains(id) { rules.push(id.clone()); }
            } else { rules.retain(|r| r != id); }
            if let Some(registration) = &registration {
                ...
                if inflight { return Err(ClientError::Busy); }
                let received: i64 = tx.query_row("SELECT COUNT(*) FROM collector_cursor
        ...
        if rules.len() > 1000 { return Err(ClientError::TooLarge); }
...
fn decode_rules(value: Option<String>) -> ClientResult<Vec<String>> {
    let rules: Vec<String> = match value {
        Some(value) if value.len() <= 1024 * 1024 => serde_json::from_str(&value).map_err(|_| ClientError::Storage)?,
        Some(_) => return Err(ClientError::Storage),
        None => vec![],
    };
    if rules.len() > 1000 || rules.iter().any(|id| !valid_id(id)) { return Err(ClientError::Storage); }
    Ok(rules)
}
