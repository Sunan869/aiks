...
pub struct ClaimedUpload{id:String,lease:String,pending:PendingSubmission,attempt:u32}
impl ClaimedUpload{
    pub fn pending(&self)->&PendingSubmission{&self.pending}
    pub fn id(&self)->&str{&self.id}
}
#[derive(Serialize)]
pub struct UploadStatus{
    pub id:String,pub state:String,pub source:String,pub attempt:u32,
    pub error_code:Option<String>,pub receipt:Option<SnapshotReceipt>,
}
...
impl CollectorOutbox{
    pub fn open(path:&Path)->ClientResult<Self>{
        ...
        let tables:i64=conn.query_row("SELECT COUNT(*) FROM sqlite_master WHERE name NOT LIKE 'sqlite_%'",[],|r|r.get(0))?;
        if !(app==APP_ID&&version==1)&&!(app==0&&version==0&&tables==0){return Err(ClientError::Storage);}
        ...
        if app==0{
            ...
            tx.pragma_update(None,"application_id",APP_ID)?;tx.pragma_update(None,"user_version",1)?;
        }
        ...
        tx.commit()?;
        Ok(Self{conn:Mutex::new(conn),_lease:lease})
    }
    fn conn(&self)->ClientResult<MutexGuard<'_,Connection>>{self.conn.lock().map_err(|_|ClientError::Storage)}
    pub fn device_id(&self)->ClientResult<String>{Ok(self.conn()?.query_row("SELECT value FROM collector_meta WHERE key='device_id'",[],|r|r.get(0))?)}
    pub(super) fn registration(&self,key:&str)->ClientResult<Option<String>>{
        ...
        if value.as_ref().is_some_and(|v|!super::valid_id(v)){return Err(ClientError::Storage);}Ok(value)
    }
    pub(super) fn remember_registration(&self,key:&str,id:&str)->ClientResult<()>{
        if key.len()>2048||!super::valid_id(id){return Err(ClientError::InvalidInput);}
        let key=format!("registration/{key}");let mut conn=self.conn()?;
        let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        ...
        if value!=id{return Err(ClientError::Conflict);}tx.commit()?;Ok(())
    }
    /// Scan continuation is independent of accepted server revisions. It never
    /// grants acknowledgement or changes a queued envelope's identity.
    pub(super) fn scan_cursor(&self,key:&str)->ClientResult<Option<String>>{
        if key.len()>128{return Err(ClientError::InvalidInput);}
        let value:Option<String>=self.conn()?.query_row("SELECT value FROM collector_meta WHERE key=?1",[format!("scan/{key}")],|r|r.get(0)).optional()?;
        if value.as_ref().is_some_and(|v|!super::valid_id(v)){return Err(ClientError::Storage);}Ok(value)
    }
    pub(super) fn save_scan_cursor(&self,key:&str,value:Option<&str>)->ClientResult<()>{
        if key.len()>128||value.is_some_and(|v|!super::valid_id(v)){return Err(ClientError::InvalidInput);}
        let key=format!("scan/{key}");
        let mut conn=self.conn()?;let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        if let Some(value)=value{
            tx.execute("INSERT INTO collector_meta(key,value) VALUES (?1,?2) ON CONFLICT(key) DO UPDATE SET value=excluded.value",params![key,value])?;
        }else{tx.execute("DELETE FROM collector_meta WHERE key=?1",[key])?;}
        tx.commit()?;Ok(())
    }
    pub fn enqueue(&self,pending:&PendingSubmission)->ClientResult<EnqueueOutcome>{
        let s=pending.submission();let mut conn=self.conn()?;
        let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        ...
        if let Some((id,hash))=exact{return if hash==pending.request_hash{Ok(EnqueueOutcome::Existing(id))}else{Err(ClientError::Conflict)};}
        ...
        if let Some((id,hash,revision))=active{return if hash==pending.content_hash&&revision==s.expected_revision{Ok(EnqueueOutcome::Existing(id))}else{Err(ClientError::Busy)};}
        ...
        if expected!=s.expected_revision{return Err(ClientError::Conflict);}
        if cursor.is_some_and(|(_,hash)|hash==pending.content_hash){return Ok(EnqueueOutcome::Unchanged);}
        ...
        if count>=MAX_PENDING_ROWS||bytes.saturating_add(pending.body.len() as i64)>MAX_PENDING_BYTES{return Err(ClientError::TooLarge);}
        ...
        tx.commit()?;Ok(EnqueueOutcome::Queued(id))
    }
    pub fn next_for(&self,instance:&str,space:&str,now:u64)->ClientResult<Option<ClaimedUpload>>{
        let now=bounded_time(now);let mut conn=self.conn()?;
        let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        ...
        type Row=(String,Vec<u8>,String,String,String,String,String,u32,u32);
        ...
        let Some((id,body,hash,content_hash,registration,upstream,submission,expected,attempt))=row else{tx.commit()?;return Ok(None);};
        if body.len()>16*1024*1024{return Err(ClientError::Storage);}
        ...
        if s.service_instance_id!=instance||s.space_id!=space||s.source_registration_id!=registration
            ||s.session.external_session_id!=upstream||s.submission_id!=submission||s.expected_revision!=expected
            ||pending.request_hash!=hash||pending.content_hash!=content_hash{return Err(ClientError::Storage);}
        pending.body=body;let attempt=attempt.checked_add(1).ok_or(ClientError::Storage)?;
        let lease=Uuid::new_v4().to_string();
        tx.execute("UPDATE collector_upload SET state='inflight',attempt=?2,lease_id=?3,lease_until=?4,error_code=NULL WHERE id=?1",params![id,attempt,lease,now.saturating_add(LEASE_MS as i64)])?;
        tx.commit()?;Ok(Some(ClaimedUpload{id,lease,pending,attempt}))
    }
    pub fn record_receipt(&self,claim:&ClaimedUpload,receipt:&SnapshotReceipt)->ClientResult<()>{
        validate_receipt(&claim.pending,receipt)?;let mut conn=self.conn()?;
        let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;require_claim(&tx,claim)?;
        ...
        if current.unwrap_or(0)!=s.expected_revision{return Err(ClientError::Conflict);}
    ...
    pub fn record_failure(&self,claim:&ClaimedUpload,error:ClientError,now:u64)->ClientResult<()>{
        let mut conn=self.conn()?;let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;require_claim(&tx,claim)?;
    ...
    pub fn revision_for(&self,instance:&str,space:&str,registration:&str,upstream:&str)->ClientResult<u32>{
    ...
    pub fn statuses(&self,instance:&str,space:&str)->ClientResult<Vec<UploadStatus>>{
        ...
        let rows=statement.query_map(params![instance,space],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,String>(2)?,r.get::<_,u32>(3)?,r.get::<_,Option<String>>(4)?,r.get::<_,Option<String>>(5)?)))?;
        let mut result=Vec::new();
        for row in rows{
            let(id,state,source,attempt,error_code,json)=row?;
            let receipt=json.map(|s|serde_json::from_str(&s)).transpose().map_err(|_|ClientError::Storage)?;
            result.push(UploadStatus{id,state,source,attempt,error_code,receipt});
        }Ok(result)
    }
}
fn bounded_time(now:u64)->i64{now.min((i64::MAX as u64)-LEASE_MS-1) as i64}
fn require_claim(tx:&rusqlite::Transaction<'_>,claim:&ClaimedUpload)->ClientResult<()>{
    let valid:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM collector_upload WHERE id=?1 AND state='inflight' AND lease_id=?2 AND request_hash=?3)",params![claim.id,claim.lease,claim.pending.request_hash],|r|r.get(0))?;
    if !valid{return Err(ClientError::Conflict);}Ok(())
}
