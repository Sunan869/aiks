...
impl CollectorOutbox {
    ...
    pub fn enqueue(&self,pending:&PendingSubmission)->ClientResult<EnqueueOutcome>{
        ...
        if let Some((id,hash))=exact {
            return if hash==pending.request_hash {Ok(EnqueueOutcome::Existing(id))}else{Err(ClientError::Conflict)};
        }
        ...
        if let Some((id,hash,revision))=active {
            return if hash==pending.content_hash && revision==s.expected_revision {Ok(EnqueueOutcome::Existing(id))}else{Err(ClientError::Busy)};
        }
    ...
    pub fn next_for(&self,instance:&str,space:&str,now:u64)->ClientResult<Option<ClaimedUpload>>{
        let now=bounded_time(now);let mut conn=self.conn()?;let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
        ...
        pending.body=body;
        let attempt=attempt.checked_add(1).ok_or(ClientError::Storage)?;let lease=Uuid::new_v4().to_string();
    ...
    pub fn record_receipt(&self,claim:&ClaimedUpload,receipt:&SnapshotReceipt)->ClientResult<()> {
        validate_receipt(&claim.pending,receipt)?;let mut conn=self.conn()?;let tx=conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
    ...
    pub fn statuses(&self,instance:&str,space:&str)->ClientResult<Vec<UploadStatus>> {
        let conn=self.conn()?;let mut statement=conn.prepare("SELECT id,state,source,attempt,error_code,receipt_json FROM collector_upload WHERE instance_id=?1 AND space_id=?2 ORDER BY rowid DESC LIMIT 100")?;
        ...
        for row in rows {let(id,state,source,attempt,error_code,json)=row?;
