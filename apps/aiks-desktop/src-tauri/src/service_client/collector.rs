...
pub async fn collect_provider(
    ...
    let (known, device) =
        blocking(move || Ok((db.registration(&lookup)?, db.device_id()?))).await?;
    ...
    for summary in discovery.sessions {
        ...
        if policy.exclude_ids.contains(upstream)
            || policy
                .include_ids
                .as_ref()
                .is_some_and(|v| !v.contains(upstream))
        {
            report.excluded += 1;
            continue;
        }
        if selected >= policy.max_sessions {
            report.deferred += 1;
