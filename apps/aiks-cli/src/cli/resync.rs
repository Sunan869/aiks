...
pub async fn run(
    ...
    engine
        .db()
        .reset_hashes_for_resync(source.as_deref(), &session_ids)?;
...
pub async fn rebuild_sync_index(_engine_config: AiksEngineConfig, yes: bool) -> anyhow::Result<()> {
    if !yes {
    ...
    println!(
        "Sync index rebuilt at {}. Knowledge data preserved.",
        db_path.display()
    );
    Ok(())
}
...
pub async fn reset_all_data(_engine_config: AiksEngineConfig, yes: bool) -> anyhow::Result<()> {
    if !yes {
