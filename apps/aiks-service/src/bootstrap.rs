...
use crate::{ServiceConfig,ServiceRuntime,LocalAuth,build_router};

#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct Bootstrap {token:String}
...
pub async fn run()->anyhow::Result<()> {
    ...
    let mut bytes=Vec::new(); file.read_to_end(&mut bytes).await?;
    ...
    drop(boot); drop(frame); drop(stdin);
    let mut stdout=tokio::io::stdout();
    stdout.write_all(format!("{ready}\n").as_bytes()).await?;
    stdout.flush().await?;
    // Client pipe EOF is intentionally not a shutdown signal.
    let server=axum::serve(listener,build_router(runtime.clone(),auth)).with_graceful_shutdown(shutdown_signal());
