...
use crate::team::{
    ...
    secrets::{resolve_secret_with, SecretSource},
};
use axum::{
...
impl Fixture {
    ...
    fn client(&self) -> DingTalkClient {
        let settings: TeamSettings = toml::from_str(
            r#"
            enabled = true
        ...
        let secret =
            resolve_secret_with(&source, |_| Some("synthetic-server-secret".into())).unwrap();
        DingTalkClient::with_test_origin(
...
async fn handle(State(state): State<Arc<FixtureState>>, request: Request) -> Response {
    ...
    if state.scenario == Scenario::Slow {
        tokio::time::sleep(Duration::from_secs(2)).await;
    }
    if state.scenario == Scenario::Redirect {
        return (
            StatusCode::TEMPORARY_REDIRECT,
            [(header::LOCATION, "/forbidden-redirect")],
        )
            .into_response();
    }
    ...
    if state.scenario == Scenario::Oversize {
        return "x".repeat(2 * 1024 * 1024 + 1).into_response();
    }
    if state.scenario == Scenario::InvalidJson {
        return "PRIVATE_INVALID_JSON".into_response();
    }
    if state.scenario == Scenario::MissingField {
        return Json(json!({"unexpected":"PRIVATE_MISSING"})).into_response();
    }
    if path.starts_with("/topapi/") {
        ...
        if state.scenario == Scenario::Permission {
            return Json(json!({"errcode":50002,"errmsg":"PRIVATE_NO_PERMISSION"})).into_response();
        }
    }
    ...
    let result = match path.as_str() {
        "/v1.0/oauth2/userAccessToken" => {
            ...
            assert_eq!(
                body,
                json!({"clientId":"synthetic-app","clientSecret":"synthetic-server-secret","code":"synthetic-code","grantType":"authorization_code"})
            );
        ...
        "/v1.0/oauth2/synthetic-corp/token" => {
            ...
            assert_eq!(
                body,
                json!({"client_id":"synthetic-app","client_secret":"synthetic-server-secret","grant_type":"client_credentials"})
            );
        ...
        "/topapi/v2/user/get" => {
            ...
            if state.scenario == Scenario::Inactive {
                user["active"] = json!(false);
            }
            if state.scenario == Scenario::WrongUnion {
                user["unionid"] = json!("another-union");
            }
        ...
        "/topapi/v2/department/listsub" => {
            let children = if body["dept_id"] == 1 {
                json!([{"dept_id":2,"parent_id":1,"name":"Research"}])
            } else {
                json!([])
            };
        ...
        "/topapi/v2/user/list" => {
            ...
            let (users, more, next) = if body["dept_id"] == 1 && body["cursor"] == 0 {
                (
                    json!([member("a")]),
                    true,
                    if state.scenario == Scenario::PaginationLoop {
                        0
                    } else {
                        100
                    },
                )
            } else if body["dept_id"] == 1 {
                (json!([member("b")]), false, 0)
            } else {
                (json!([member("a")]), false, 0)
            };
...
async fn real_wire_verifies_company_union_and_membership_without_credential_forwarding() {
    ...
    assert_eq!(user.corp_id, "synthetic-corp");
    assert_eq!(user.union_id, "union-a");
    assert_eq!(user.external_user_id, "a");
...
async fn foreign_company_disabled_member_and_identity_mismatch_are_rejected() {
    for scenario in [
        ...
        if scenario == Scenario::WrongCorp {
            assert_eq!(f.state.seen.lock().await.len(), 1);
        }
    }
}
...
async fn complete_directory_paginates_and_keeps_multi_department_memberships() {
    ...
    assert_eq!(snapshot.scope, vec!["1"]);
    assert_eq!(snapshot.orgs.len(), 2);
    ...
    assert_eq!(
        f.state
            ...
            .await
            .iter()
            .filter(|(p, _)| p == "/topapi/v2/user/list")
            .count(),
        3
    );
    let root = tempfile::tempdir().unwrap();
    let store = aiks_core::team::TeamStore::bind(
        Arc::new(
            aiks_core::storage::StateDb::open_exclusive(&root.path().join("team.db")).unwrap(),
        ),
        "synthetic-corp",
        "synthetic-app",
    )
    .unwrap();
    let time = snapshot.observed_at;
    store.publish_directory(snapshot, time).unwrap();
...
async fn upstream_failures_are_bounded_and_never_echo_private_response_text() {
    for scenario in [
        ...
        for private in [
            "PRIVATE_",
            "synthetic-server-secret",
            "synthetic-user-token",
            &f.base,
        ] {
            assert!(!printable.contains(private));
        }
...
async fn empty_or_invalid_input_never_calls_upstream() {
    ...
    for scope in [
        ...
        assert!(client.directory(&scope).await.is_err());
    }
