...
use super::{config::{validate_static, ConfigIssue}, secrets::resolve_secret_with};
...
impl ServiceConfig {
    pub fn check_configuration_with(
        ...
        match self.mode.as_str() {
            "personal" => self.validate().map_err(|_| issue("service", "invalid_personal_configuration"))?,
            "team" => {
                let validated = validate_static(&self.team)
                    .map_err(|issues| issues[0])?;
                if self.ai.enabled && (self.ai.base_url.trim().is_empty() || self.ai.model.trim().is_empty()) {
                    return Err(issue("ai", "explicit_endpoint_and_model_required"));
                }
                if self.embedding.enabled && (self.embedding.base_url.trim().is_empty() || self.embedding.model.trim().is_empty()) {
        ...
        candidate.resolve_model_credentials_with(lookup)
