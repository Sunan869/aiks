...
async fn owned_service_restarts_with_same_identity_and_cannot_stop_a_foreign_writer() {
    ...
    assert_eq!(
        client.capabilities().await.unwrap()["instance_id"],
        identity
    );
    ...
    let mut restarted = OwnedService::start(binary, &config, Some(&identity))
        ...
        .unwrap();
    assert_eq!(
        restarted.client().capabilities().await.unwrap()["instance_id"],
        identity
    );
...
async fn explicitly_managed_parent_pipe_eof_drains_but_independent_mode_stays_alive() {
    for managed in [true, false] {
        ...
        pipe.write_all(
            format!(
                "{}\n",
                json!({"token":"ab".repeat(32),"owner_control":managed,"owner_lifetime":managed})
            )
            .as_bytes(),
        )
        .await
        ...
        drop(pipe);
        if managed {
            assert!(tokio::time::timeout(Duration::from_secs(12), child.wait())
                .await
                .unwrap()
                .unwrap()
                .success());
