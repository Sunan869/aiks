...
use aiks_core::{
    ...
    model::SourceKind,
    providers::{claude::ClaudeProvider, native::NativeProvider, SessionProvider},
};
...
async fn real_provider_is_sanitized_before_queueing_and_survives_source_removal() {
    ...
    let tools = root.path().join("continue");
    write_continue(
        &tools,
        "one",
        "COLLECTOR_UNIQUE token=SYNTHETIC_CAPTURE_12345",
    );
    ...
    let report = collect_provider(&provider, &c, o.clone(), &CollectionPolicy::default())
        .await
        .unwrap();
    assert_eq!((report.queued, report.failed), (1, 0));
    assert!(report.complete);
    let second = collect_provider(&provider, &c, o.clone(), &CollectionPolicy::default())
        .await
        .unwrap();
    ...
    assert!(body.contains("COLLECTOR_UNIQUE"));
    assert!(!body.contains("SYNTHETIC_CAPTURE_12345"));
    ...
    let receipt = deliver_one(&c, o.clone(), 1_000_000)
        .await
        .unwrap()
        .unwrap();
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            let job = c.get_job(&receipt.job_id).await.unwrap();
            if job["status"] == "DONE" {
                break;
            }
    ...
    .unwrap();
    assert_eq!(
        c.search("COLLECTOR_UNIQUE").await.unwrap()["hits"]
            .as_array()
            .unwrap()
            .len(),
        1
...
async fn a_known_registration_can_queue_offline_and_excluded_sessions_are_not_uploaded() {
    ...
    let root = tempfile::tempdir().unwrap();
    let tools = root.path().join("continue");
    write_continue(&tools, "one", "FIRST");
    let provider = native(&tools);
    let c = ServiceClient::new(
        ServiceConnection::local(&s.base, &s.instance_id, &s.space_id, &s.token).unwrap(),
    )
    ...
    assert_eq!(
        collect_provider(&provider, &c, o.clone(), &policy)
            .await
            .unwrap()
            .queued,
        1
    ...
    let old_id = provider.discover_sessions().await.unwrap()[0]
        .external_session_id
        .clone();
    s.stop().await;
    write_continue(&tools, "two", "OFFLINE_QUEUED");
    let policy = CollectionPolicy {
        exclude_ids: vec![old_id],
        ..Default::default()
    };
    let report = collect_provider(&provider, &c, o.clone(), &policy)
        .await
        .unwrap();
    assert_eq!(report.queued, 1);
    assert_eq!(report.excluded, 1);
    let rows = o
        .statuses(c.connection().instance_id(), c.connection().space_id())
        .unwrap();
...
async fn malformed_legacy_jsonl_is_not_labeled_a_complete_snapshot() {
    let s = RunningService::start().await;
    let root = tempfile::tempdir().unwrap();
    let tools = root.path().join("claude");
    let project = tools.join("projects").join("-synthetic");
    ...
    .unwrap();
    let o = Arc::new(CollectorOutbox::open(&root.path().join("collector.db")).unwrap());
    let result = collect_provider(&provider, &c, o.clone(), &CollectionPolicy::default())
        .await
        .unwrap();
    assert_eq!(result.queued, 0);
    assert_eq!(result.failed, 1);
    assert!(!result.complete);
    let rows: Vec<UploadStatus> = o.statuses(&s.instance_id, &s.space_id).unwrap();
    assert!(rows.is_empty());
    let count: Value = c.sessions().await.unwrap();
