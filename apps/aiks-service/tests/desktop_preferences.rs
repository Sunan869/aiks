...
use aiks_core::{
    ...
    model::SourceKind,
    providers::{native::NativeProvider, SessionProvider},
};
use serde_json::json;
...
use service_client::{ClientError, CollectorOutbox, ServiceClient, ServiceConnection};
use std::sync::Arc;
mod support;
...
fn provider(root: &std::path::Path) -> NativeProvider {
    ...
    for (id, title, text) in [
        (
            "one",
            "项目迁移",
            "READ_ONLY_PREVIEW token=SYNTHETIC_SECRET_12345",
        ),
        ("two", "临时测试", "SECOND_SESSION"),
    ] {
...
fn skipping_onboarding_is_durable_without_a_service_connection_or_any_upload() {
    ...
    assert_eq!(
        store.ui_preferences().unwrap().onboarding,
        Onboarding::Unseen
    );
...
async fn scanning_and_preview_only_read_local_data_and_use_opaque_row_keys() {
    ...
    assert_eq!(page.items[0].title, "项目迁移");
    assert!(!serde_json::to_string(&page)
        .unwrap()
        .contains("external_session_id"));
    assert!(!serde_json::to_string(&page)
        .unwrap()
        .contains("source_path"));
    let preview = browser
        .preview(&provider, &page.items[0].key)
        .await
        .unwrap();
    let body = serde_json::to_string(&preview).unwrap();
    assert!(body.contains("READ_ONLY_PREVIEW"));
    assert!(!body.contains("SYNTHETIC_SECRET_12345"));
...
async fn exclusion_stops_unsent_content_is_source_scoped_and_survives_restart() {
    ...
    .unwrap();
    let path = root.path().join("collector.db");
    let store = Arc::new(CollectorOutbox::open(&path).unwrap());
    let policy = CollectionPolicy::default();
    collect_provider(&provider, &client, store.clone(), &policy)
        .await
        .unwrap();
    let scope =
        SourceScope::new(&s.instance_id, &s.space_id, SourceKind::Continue, "default").unwrap();
    let one = provider
        .discover_sessions()
        .await
        .unwrap()
        .into_iter()
        .find(|r| r.title.as_deref() == Some("项目迁移"))
        .unwrap()
        .external_session_id;
    let change = store
        .set_excluded(&scope, std::slice::from_ref(&one), true)
        .unwrap();
    assert_eq!(change.paused, 1);
    assert_eq!(change.already_received, 0);
    let other = SourceScope::new(
        &s.instance_id,
        &s.space_id,
    ...
    let report = collect_provider(&provider, &client, store.clone(), &policy)
        .await
        .unwrap();
    assert_eq!(
        report.excluded, 1,
        "collector must honor saved rules without the UI resending IDs"
    );
    let receipt = deliver_one(&client, store.clone(), 0)
        .await
        .unwrap()
        .unwrap();
    let session = client.session(&receipt.session_id).await.unwrap();
    assert!(session.to_string().contains("SECOND_SESSION"));
    assert!(deliver_one(&client, store.clone(), 0)
        .await
        .unwrap()
        .is_none());
    store
        .set_excluded(&scope, std::slice::from_ref(&one), false)
        .unwrap();
    let receipt = deliver_one(&client, store.clone(), 0)
        .await
        .unwrap()
    ...
    let change = store
        .set_excluded(&scope, std::slice::from_ref(&one), true)
        .unwrap();
    assert_eq!(change.paused, 0);
    assert!(change.already_received > 0);
    assert!(
        client.session(&receipt.session_id).await.is_ok(),
        "excluding does not delete server knowledge"
    );
    s.stop().await;
}
...
async fn exclusion_does_not_claim_to_recall_an_inflight_request() {
    ...
    .unwrap();
    let claim = store
        .next_for(&s.instance_id, &s.space_id, 0)
        .unwrap()
        .unwrap();
    let id = claim
        .pending()
    ...
    let scope =
        SourceScope::new(&s.instance_id, &s.space_id, SourceKind::Continue, "default").unwrap();
    assert!(matches!(
        store.set_excluded(&scope, &[id], true),
