...
use aiks_core::{
    ...
    config::{ExternalProviderConfig, SiYuanConfig},
    indexing::{KnowledgeIndexInput, KnowledgeIndexService},
    knowledge::publisher::publish_knowledge_to_siyuan,
    model::SourceKind,
    pipeline::EmbeddingConfig,
    providers::native::NativeProvider,
    sink::SiYuanSink,
    storage::StateDb,
};
...
async fn fixture_request(
    State(state): State<Arc<Fixtures>>,
    uri: Uri,
    ...
    let response = match path {
        ...
        "/api/notebook/lsNotebooks" => {
            json!({"code":0,"data":{"notebooks":[{"id":"kn","name":"AI Knowledge","closed":false}]}})
        }
        "/api/query/sql" => {
        ...
        "/api/block/getBlockKramdown" => {
            ...
            let body = state
                ...
                .get(id)
                .cloned()
                .expect("only mapped documents may be fetched");
...
async fn actual_binary_keeps_processing_without_source_and_reuses_canonical_publisher_after_restart(
    ...
    if std::env::var_os("AIKS_TEST_ISOLATED_NETWORK").is_some() {
        let external = tokio::time::timeout(
            ...
            tokio::net::TcpStream::connect("192.0.2.1:80"),
        )
        .await;
        assert!(
            matches!(external, Ok(Err(_))),
            "offline gate requires a namespace with no external route"
        );
    }
    let fixture = Arc::new(Fixtures {
        documents: Mutex::new(HashMap::new()),
        model_seen: AtomicBool::new(false),
        model_gate: Semaphore::new(0),
    });
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let origin = format!("http://{}", listener.local_addr().unwrap());
    let app = Router::new()
        .fallback(fixture_request)
    ...
    tokio::time::timeout(Duration::from_secs(20), async {
        loop {
            ...
            if job["status"] == "DONE" {
                assert_eq!(job["pipeline_status"], "READY");
                break;
            }
    ...
    assert!(observer
        ...
        .unwrap()
        .to_string()
        .contains("OFFLINE_CANONICAL_SESSION"));
    ...
    let sink = SiYuanSink::new(SiYuanConfig {
        base_url: origin.clone(),
        ..Default::default()
    })
    .unwrap();
    let published = publish_knowledge_to_siyuan(&db, &sink, &id, false)
        .await
        .unwrap();
    ...
    KnowledgeIndexService::new(db.clone(), models)
        ...
        .unwrap();
    drop(db);
    let mut restarted = OwnedService::start(binary, &config, Some(&identity))
        .await
        .unwrap();
    ...
    assert!(result["hits"]
        ...
        .unwrap()
        .iter()
        .any(|hit| hit["entity_id"] == id && hit["corpus"] == "knowledge"));
    let body = client.knowledge(&id).await.unwrap();
    assert_eq!(body["content_state"], "published");
    assert!(body["content"]
        .as_str()
        .unwrap()
        .contains("OFFLINE_EXTRACTED_KNOWLEDGE"));
