...
use axum::{http::{HeaderMap, StatusCode}, response::Redirect, routing::post, Json, Router};
...
async fn published_body_comes_from_fixed_content_api_and_draft_is_explicit() {
    let remote = upstream(Router::new().route("/api/block/getBlockKramdown", post(|headers: HeaderMap, Json(value): Json<Value>| async move {
    ...
    let response = s.auth(s.client.get(format!("{}/api/v1/knowledge/published", s.base))).send().await.unwrap();
    assert_eq!(response.status(), 200);
    let text = response.text().await.unwrap();
    assert!(text.contains("CANONICAL_REMOTE_BODY"));
    for secret in ["SQLITE_DRAFT_ONLY", "mapped-doc", "synthetic-content-secret", &remote.base] { assert!(!text.contains(secret)); }
    let draft: Value = s.auth(s.client.get(format!("{}/api/v1/knowledge/draft", s.base))).send().await.unwrap().json().await.unwrap();
...
async fn unavailable_published_body_is_not_replaced_with_the_sqlite_projection() {
    let remote = upstream(Router::new().fallback(|| async { (StatusCode::SERVICE_UNAVAILABLE, "PRIVATE_UPSTREAM_ERROR") })).await;
    let s = RunningService::configured(|config| { config.siyuan.base_url = remote.base.clone(); }).await;
    let receipt = s.ingest_ready("Synthetic source").await;
    s.seed_knowledge(&receipt, "published", Some("mapped-doc"));
    let response = s.auth(s.client.get(format!("{}/api/v1/knowledge/published", s.base))).send().await.unwrap();
...
async fn disabled_ai_does_not_contact_content_or_model_services() {
    ...
    assert_eq!(response.status(), 503);
    assert_eq!(response.json::<Value>().await.unwrap()["error"]["code"], "ai_disabled");
    assert_eq!(seen.load(Ordering::SeqCst), 0);
    let response = s.auth(s.client.post(format!("{}/api/v1/knowledge/nonexistent/assist", s.base)))
        .json(&json!({"operation":"summary"})).send().await.unwrap();
...
async fn content_read_rechecks_visibility_without_holding_a_database_lock_across_await() {
    ...
    let receipt = s.ingest_ready("Synthetic source").await;
    s.seed_knowledge(&receipt, "published", Some("mapped-doc"));
    let request = s.auth(s.client.get(format!("{}/api/v1/knowledge/published", s.base)));
    let task = tokio::spawn(async move { request.send().await.unwrap() });
    tokio::time::timeout(Duration::from_secs(2), seen.acquire()).await.unwrap().unwrap().forget();
    let other = tokio::time::timeout(Duration::from_secs(1), s.auth(s.client.get(format!("{}/api/v1/sessions", s.base))).send()).await.unwrap().unwrap();
    assert_eq!(other.status(), 200);
    rusqlite::Connection::open(&s.path).unwrap().execute("UPDATE knowledge_item SET status='archived' WHERE id='published'", []).unwrap();
