...
use service_client::{CollectorOutbox,ServiceClient,ServiceConnection,collector::{CollectionPolicy,collect_provider}};
use aiks_core::{config::ExternalProviderConfig,model::SourceKind,providers::native::NativeProvider};
use serde_json::json;
use std::sync::Arc;
mod support;
...
async fn repeated_bounded_capture_advances_past_unchanged_sessions_and_survives_restart() {
    ...
    let client=ServiceClient::new(ServiceConnection::local(&service.base,&service.instance_id,&service.space_id,&service.token).unwrap()).unwrap();
    let queue=root.path().join("collector.db");
    let policy=CollectionPolicy{max_sessions:3,..Default::default()};
    let mut counts=Vec::new();
    for _ in 0..3 {
        let outbox=Arc::new(CollectorOutbox::open(&queue).unwrap());
        let report=collect_provider(&provider,&client,outbox,&policy).await.unwrap();
        counts.push(report.queued);
    }
    assert_eq!(counts,vec![3,3,1],"a later batch must not rescan only the first unchanged entries");
    let outbox=Arc::new(CollectorOutbox::open(&queue).unwrap());
    let report=collect_provider(&provider,&client,outbox.clone(),&policy).await.unwrap();
    assert_eq!(report.queued,0);
    assert_eq!(outbox.statuses(&service.instance_id,&service.space_id).unwrap().len(),7);
    service.stop().await;
}
