...
async fn owned_pipe_shutdown_is_explicit_and_releases_the_writer_on_all_platforms() {
    ...
    stdin.write_all(format!("{}\n",json!({"token":"ab".repeat(32),"owner_control":true})).as_bytes()).await.unwrap();
    let mut stdout=BufReader::new(child.stdout.take().unwrap());
    let mut line=String::new();
    tokio::time::timeout(Duration::from_secs(10),stdout.read_line(&mut line)).await.unwrap().unwrap();
    let ready:Value=serde_json::from_str(&line).expect("owned service must produce a ready handshake");
    assert!(StateDb::open_exclusive(&db).is_err());
    stdin.write_all(format!("{}\n",json!({"command":"shutdown","boot_nonce":ready["boot_nonce"]})).as_bytes()).await.unwrap();
    let exit=tokio::time::timeout(Duration::from_secs(12),child.wait()).await.expect("explicit owner shutdown must not require a forced kill").unwrap();
    assert!(exit.success());
