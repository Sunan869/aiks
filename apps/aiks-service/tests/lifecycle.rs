...
fn public_listen_and_team_mode_are_rejected_before_opening_a_database() {
    ...
    for address in [
        ...
        assert!(LocalAuth::new(&"ab".repeat(32), "test-instance")
            .unwrap()
            .with_authority(config.listen)
            .is_err());
    }
...
async fn independent_process_survives_stdin_eof_and_restart_keeps_identity() {
    ...
    for _ in 0..2 {
        ...
        if let Some((old_id, old_nonce)) = previous {
            assert_eq!(identity, old_id);
        ...
        let response = reqwest::Client::builder()
            ...
            .bearer_auth(&token)
            .header("X-AIKS-Instance-ID", identity)
            .send()
            .await
            .unwrap();
        assert_eq!(
            response.status(),
            200,
            "stdin EOF must not shut down the service"
        );
        assert!(StateDb::open_exclusive(&database).is_err());
        child.kill().await.unwrap();
        child.wait().await.unwrap();
        assert!(
            StateDb::open_exclusive(&database).is_ok(),
            "OS must release the writer lock"
        );
    }
}
