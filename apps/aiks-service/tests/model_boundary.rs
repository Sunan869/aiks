...
async fn enabled_assist_uses_canonical_content_and_returns_an_unsaved_suggestion() {
    ...
    assert_eq!(
        output["suggestion"]["summary"],
        "Canonical summary suggestion"
    );
    assert_eq!(seen.load(Ordering::SeqCst), 2);
    let stored: String = db
        .query_row(
            "SELECT summary FROM knowledge_item WHERE id='published'",
            [],
            |row| row.get(0),
        )
        .unwrap();
...
async fn semantic_recall_scopes_both_corpora_before_the_vector_candidate_limit() {
    let remote = upstream(Router::new().route(
        ...
        post(|Json(input): Json<Value>| async move {
            let data: Vec<Value> = input["input"]
                .as_array()
                .unwrap()
                .iter()
                .enumerate()
    ...
    .await;
    let vector: Vec<u8> = [1.0_f32, 0.0, 0.0]
        .iter()
        ...
        .collect();
    {
        ...
        tx.execute(
            "INSERT INTO session_index_state(session_id,status) VALUES (?1,'ready')",
            [id],
        )
        .unwrap();
        ...
        for n in 0..4100 {
            ...
            tx.execute(
                "INSERT INTO embedding_record(id,chunk_id,model,dimensions,vector,created_at)
                        VALUES (?1,?1,'synthetic-vector',3,?2,'test')",
                params![key, vector],
            )
            .unwrap();
        }
    ...
    db.execute(
        "INSERT INTO embedding_record(id,chunk_id,model,dimensions,vector,created_at)
                VALUES ('authorized-vector','authorized-chunk','synthetic-vector',3,?1,'test')",
        [&vector],
    )
    .unwrap();
    for stale in [false, true] {
        if stale {
            db.execute(
                "DELETE FROM service_knowledge_revision WHERE knowledge_id='authorized-knowledge'",
                [],
            )
            .unwrap();
        }
        let response = s
            .auth(s.client.post(format!("{}/api/v1/search", s.base)))
            .json(&json!({"query":"vectoronlyneedle","limit":10}))
            .send()
            .await
            .unwrap();
        assert_eq!(response.status(), 200);
        ...
        assert!(hits
            .iter()
            .any(|hit| hit["entity_id"] == receipt["session_id"] && hit["corpus"] == "session"));
        for hit in hits {
            assert!(hit["match_types"]
                .as_array()
                .unwrap()
