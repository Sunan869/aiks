...
use aiks_service::{team::secrets::{resolve_secret_with, SecretSource}, ServiceConfig};
...
fn server_secret_resolution_is_injected_bounded_and_redacted() {
    ...
    assert_eq!(resolve_secret_with(&source, |_| None).unwrap_err().code, "secret_missing");
    for invalid in [String::new(), "a\nb".into(), "a\0b".into(), " leading".into(), "trailing ".into(), "x".repeat(8193)] {
    ...
    assert!(resolve_secret_with(&invalid, |_| panic!("invalid reference must not be looked up")).is_err());
    // File-reference support remains explicitly unavailable until OS-specific
    // ownership/ACL validation is implemented and verified. Never silently read.
    let file = SecretSource::File(std::env::temp_dir().join("not-created-secret"));
    assert_eq!(resolve_secret_with(&file, |_| panic!("file is not env")).unwrap_err().code, "secret_file_not_supported");
}
...
fn personal_check_ignores_dingtalk_and_does_not_fall_back_from_team() {
    ...
    config.check_configuration_with(|_| panic!("personal mode needs no DingTalk credentials")).unwrap();
    config.mode = "team".into();
    assert!(config.check_configuration_with(|_| panic!("disabled team must not look up secrets")).is_err());
...
"#, serde_json::to_string(&database).unwrap(), listener.local_addr().unwrap());
    std::fs::write(&path, &text).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_aiks-service"))
        .args(["--check-config", "--config"]).arg(&path)
        .env("AIKS_TEST_CHECK_SECRET", "SYNTHETIC_CHECK_VALUE").output().unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_aiks-service"))
        .args(["--config"]).arg(&path).arg("--check-config")
        .env_remove("AIKS_TEST_CHECK_SECRET").output().unwrap();
