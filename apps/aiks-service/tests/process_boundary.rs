...
async fn binary_rejects_team_and_nonloopback_without_opening_the_database() {
    for extra in ["mode='team'", "listen='0.0.0.0:8080'"] {
        ...
        let output = tokio::time::timeout(Duration::from_secs(5),child.wait_with_output()).await.unwrap().unwrap();
        assert!(!output.status.success());
        assert!(output.stdout.is_empty());
        assert!(!database.exists());
        assert_eq!(String::from_utf8(output.stderr).unwrap().trim(),"AIKS service startup or shutdown failed");
    }
}
...
async fn graceful_stop_releases_writer_and_startup_does_not_create_provider_directories() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_aiks-service"))
        .args(["--config",config.to_str().unwrap(),"--bootstrap-stdin"])
        .env("HOME",&home).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::null())
        .kill_on_drop(true).spawn().unwrap();
    let mut stdin = child.stdin.take().unwrap();
    stdin.write_all(format!("{}\n",json!({"token":"ef".repeat(32)})).as_bytes()).await.unwrap();
    drop(stdin);
    let mut stdout = BufReader::new(child.stdout.take().unwrap());
    let mut line = String::new();
    tokio::time::timeout(Duration::from_secs(5),stdout.read_line(&mut line)).await.unwrap().unwrap();
    ...
    let signal = Command::new("kill").args(["-TERM",&child.id().unwrap().to_string()]).status().await.unwrap();
