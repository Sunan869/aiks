...
fn snapshot(complete: bool) -> DirectorySnapshot {
    DirectorySnapshot {
        ...
        users: vec![DirectoryUser {
            external_user_id: "employee".into(),
            union_id: "union-employee".into(),
            display_name: "Employee".into(),
            active: true,
        }],
        memberships: vec![Membership {
            user_id: "employee".into(),
            org_id: "1".into(),
        }],
...
struct Provider {
    replies: Mutex<VecDeque<Result<DirectorySnapshot, TeamError>>>,
    active: AtomicUsize,
    calls: AtomicUsize,
}
...
impl IdentityProvider for Provider {
    fn exchange_code<'a>(&'a self, _: &'a str) -> ProviderFuture<'a, ExternalLogin> {
        Box::pin(async { Err(TeamError::Unauthorized) })
    }
...
fn fixture() -> (tempfile::TempDir, Arc<TeamStore>, Arc<Provider>) {
    ...
    let store = Arc::new(
        TeamStore::bind(
            Arc::new(StateDb::open_exclusive(&root.path().join("company.db")).unwrap()),
            "synthetic-corp",
            "synthetic-app",
        )
        .unwrap(),
...
async fn settled(worker: &DirectoryWorker, attempt: u64) {
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            ...
            if state.attempt >= attempt && !state.running {
                break;
            }
...
async fn failed_and_partial_refresh_preserve_the_last_complete_directory() {
    ...
    provider.replies.lock().await.extend([
        Ok(snapshot(true)),
    ...
    worker.request_refresh();
    settled(&worker, 4).await;
    assert_eq!(store.directory_generation().unwrap(), 2);
    assert_eq!(
        store.user_by_union("union-employee").unwrap().unwrap().id,
        id
    );
    assert!(worker.status().error_code.is_none());
    let failed: i64 = store
        ...
        .conn()
        .query_row(
            "SELECT COUNT(*) FROM team_audit_event WHERE action='directory_refresh_failed'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
async fn refresh_is_serial_coalesced_and_shutdown_cancels_an_inflight_upstream_call() {
    ...
    worker.shutdown(Duration::from_secs(2)).await.unwrap();
    assert_eq!(provider.active.load(Ordering::SeqCst), 0);
    assert_eq!(store.directory_generation().unwrap(), 0);
    worker.shutdown(Duration::from_secs(2)).await.unwrap();
}
...
async fn invalid_policy_or_mismatched_scope_cannot_publish_another_scope() {
    ...
    wrong.refresh_seconds = 0;
    assert!(DirectoryWorker::start(store.clone(), provider.clone(), wrong).is_err());
    assert_eq!(provider.calls.load(Ordering::SeqCst), 0);
    let mut mismatched = snapshot(true);
    mismatched.scope = vec!["2".into()];
