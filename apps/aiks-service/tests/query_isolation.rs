...
async fn unbound_legacy_rows_cannot_fill_the_candidate_budget_or_leak_via_fallbacks() {
    ...
    assert!(search(&s, "PRIVATE_LEGACY").await["hits"]
        .as_array()
        .unwrap()
        .is_empty());
    let db = Connection::open(&s.path).unwrap();
    db.execute(
        "UPDATE source_session SET title='scopeprobe Current title' WHERE id=?1",
    ...
    assert!(!result.to_string().contains("PRIVATE_LEGACY"));
    db.execute("UPDATE service_derived_state SET indexed_revision=NULL", [])
        .unwrap();
    assert!(search(&s, "scopeprobe").await["hits"]
        .as_array()
        .unwrap()
        .is_empty());
    drop(db);
...
async fn stale_index_never_combines_old_body_with_current_metadata() {
    ...
    db.execute("UPDATE service_derived_state SET indexed_revision=NULL", [])
        .unwrap();
    db.execute(
        "UPDATE source_session SET title='New unindexed title' WHERE id=?1",
        [receipt["session_id"].as_str().unwrap()],
    )
    .unwrap();
    ...
    let list: Value = s
        .auth(s.client.get(format!("{}/api/v1/sessions", s.base)))
        .send()
        .await
        .unwrap()
        .json()
...
async fn foreign_instance_receipts_jobs_and_unbound_knowledge_are_not_readable() {
    ...
    for (path, field) in [
        ...
        let url = format!(
            "{}/api/v1/{path}/{}",
            b.base,
            receipt[field].as_str().unwrap()
        );
        assert_eq!(
            b.auth(b.client.get(&url)).send().await.unwrap().status(),
            404
        );
        assert_eq!(
            b.client
                .get(&url)
                .bearer_auth(&a.token)
                .header("X-AIKS-Instance-ID", &a.instance_id)
                .send()
                .await
                .unwrap()
                .status(),
    ...
    assert!(search(&b, "PRIVATE_INSTANCE_A").await["hits"]
        .as_array()
        .unwrap()
        .is_empty());
    b.stop().await;
