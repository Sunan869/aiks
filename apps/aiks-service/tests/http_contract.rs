...
async fn authenticated_upload_replay_worker_and_keyword_search_form_a_real_offline_flow() {
    ...
    assert_eq!(
        unchanged.status(),
        StatusCode::ACCEPTED,
        "new receipt is not an exact replay"
    );
...
async fn authentication_host_origin_instance_and_route_boundaries_are_enforced() {
    ...
    let requests = vec![
        ...
        s.client
            .get(&url)
            .bearer_auth(&s.token)
            .header("X-AIKS-Instance-ID", "wrong-instance"),
        s.auth(s.client.get(&url))
    ...
    for path in [
        ...
        let response = s
            .auth(s.client.post(format!("{}{path}", s.base)))
            .send()
            .await
            .unwrap();
    ...
    assert_eq!(
        s.auth(s.client.get(format!("{url}?token=forbidden")))
            .send()
            .await
            .unwrap()
            .status(),
...
async fn invalid_requests_are_bounded_and_do_not_echo_private_payloads() {
    ...
    for (change, status, code) in [
        ...
        (
            json!({"unexpected":"PRIVATE_SYNTHETIC_NEEDLE"}),
            400,
            "invalid_input",
        ),
    ] {
    ...
    let response = s
        .auth(
            s.client
                .get(format!("{}/api/v1/sessions?limit=101", s.base)),
        )
        .send()
        .await
        .unwrap();
